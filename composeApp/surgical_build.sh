...
echo ">>> 1. Перезаписываем воркфлоу на режим 'без демона'..."
...
jobs:
  build:
    ...
    - name: Build with No-Daemon
      run: ./gradlew assembleDebug --no-daemon --stacktrace
      env:
        GRADLE_OPTS: "-Xmx3072m -Dorg.gradle.daemon=false"
YML

echo ">>> 2. Форсированный пуш..."
git config --global --add safe.directory "$PROJECT_ROOT"
git remote set-url origin "https://$GH_USER:$GH_TOKEN@$REPO_URL"
git add .
git commit -m "Build: Disable daemon and set surgical limits"
git push origin main --force

echo ">>> ГОТОВО! Билд пошел. Теперь в логах GitHub ищи не хвост, а самое НАЧАЛО ошибки."
